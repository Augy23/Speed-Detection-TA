...
# Main camera processing function with display
def process_camera(camera_index, camera_name):
    global system_running
    log_activity(f"Starting camera {camera_index} as '{camera_name}'", send_to_telegram=True)
    ...
    if camera_name not in system_stats["cameras"]:
        system_stats["cameras"][camera_name] = {
            ...
            "vehicles_detected": 0,
            "violations": 0,
            "people_detected": 0,  # Added people detection counter
    ...
    while cap.isOpened() and system_running:
        ...
        if not ret:
            system_stats["cameras"][camera_name]["active"] = False
            log_activity(f"Camera {camera_name} disconnected", send_to_telegram=True)
            break
    ...
    system_stats["cameras"][camera_name]["active"] = False
    log_activity(f"Camera {camera_name} stopped", send_to_telegram=True)
    cap.release()
    cv2.destroyWindow(camera_name)
